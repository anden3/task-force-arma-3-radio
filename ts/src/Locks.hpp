...
class ReadWriteLock {  //Consider SRW locks if you are reading at least 4:1 vs writing
    ...
    std::shared_mutex m_lock/*{ SRWLOCK_INIT }*/;
};
...
public:
    explicit LockGuard_exclusive(CriticalSectionLock* _cs) : m_lock(_cs) {
        // EnterCriticalSection(&m_lock->m_lock);
        m_lock->m_lock.lock();
        isLocked = true;
    }
...
public:
    explicit LockGuard_exclusive(ReadWriteLock* _cs) : m_lock(_cs) {
        // AcquireSRWLockExclusive(&m_lock->m_lock);
        m_lock->m_lock.lock();
        isLocked = true;
    }
...
public:
    explicit LockGuard_shared(CriticalSectionLock* _cs) : m_lock(_cs) {
        // EnterCriticalSection(&m_lock->m_lock);
        m_lock->m_lock.lock();
        isLocked = true;
    }
...
public:
    explicit LockGuard_shared(ReadWriteLock* _cs) : m_lock(_cs) {
        // AcquireSRWLockShared(&m_lock->m_lock);
        m_lock->m_lock.lock_shared();
        isLocked = true;
    }
